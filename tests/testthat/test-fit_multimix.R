...
test_that(
  "fit_multimix_lite returns correct class and structure with sample_data",
  {
    ...
    expect_equal(
      length(result$u_hat), length(unique(sample_data$Subject_ID))
    )
...
test_that("fit_multimix returns correct class and structure", {
  ...
  expect_equal(
    nrow(result$u_hat), length(unique(sample_data$Subject_ID))
  )
